...
`docling-standard` pins Docling `2.123.0` and adapter `0.1.1`. It enables RapidOCR with the Chinese `ch` profile on the Torch backend, accurate TableFormer structure with cell matching, and local code/formula enrichment. Remote services, external plug-ins and picture-description models are disabled. `device=auto` resolves to CUDA only when available and otherwise to CPU; explicit unavailable CUDA is a runtime error, not a document-quality failure. Page/model batch size is one in this correctness-first slice. This profile is development evidence only until model licenses and Golden Dataset thresholds are approved. Adapter 0.1.1 preserves Docling's distinct row/column header evidence.
...
`paddleocr-vl-1.6` pins `paddleocr==3.7.0`, `paddlex==3.7.1`, PaddlePaddle `3.3.0`, adapter `0.1.3`, PP-DocLayoutV3, and PaddleOCR-VL-1.6-0.9B. It integrates the complete layout/crop/order/recognition/assembly pipeline rather than the bare VLM. PaddlePaddle is installed separately from the official device-specific wheel index; CUDA imports and SDK objects remain inside the adapter boundary. Adapter 0.1.3 preserves `<th>` as role-unknown header evidence rather than inventing a row/column axis and explicitly maps the pinned PaddleX layout-label vocabulary at the adapter boundary. Known labels without an exact neutral semantic type, such as `formula_number`, remain `UNKNOWN` with their raw Paddle label retained. Unmapped future labels emit deterministic page-scoped warnings instead of failing or silently disappearing.
...
Paddle `parsing_res_list` is mapped as structured blocks. Explicit `parent_block_id` links a caption/title block to a table or figure only when the referenced parent resolves to that compatible type; no geometry or text heuristic is used. Table HTML is parsed deterministically into logical cells while preserving `rowspan` and `colspan`; absent cell geometry remains `null`. Markdown is a debug/derived artifact and is not the neutral contract.
